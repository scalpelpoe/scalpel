...
const activate: PluginActivate = (ctx) => {
  ctx.registerTab({
    ...
    render(container) {
      ...
      `
      const button = container.querySelector<HTMLButtonElement>('[data-open-native-overlay]')!
      const open = (): void => ctx.openOverlay()
    ...
    (container) => {
      ...
      const analyze = async (): Promise<void> => {
        ...
        try {
          ...
          container.querySelector<HTMLElement>('[data-result-fingerprint]')!.textContent = analysis.fingerprint
