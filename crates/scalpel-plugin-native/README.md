...
**Experimental preview:** RFC1 and this crate can be replaced by a later protocol version. The crate is not published to crates.io. Pin the SDK release tag that contains the current RFC1 implementation:
...
`serve_stdio` validates the initialization identity, enforces the 1 MiB envelope limit, writes transport errors, flushes each response, and exits normally at stdin EOF. A reply over 1 MiB is answered with a `RESOURCE_EXHAUSTED` `CallError` instead of terminating the worker. Its dispatcher is synchronous and sequential. Write diagnostics to stderr only; stdout is reserved for framed protocol messages.
