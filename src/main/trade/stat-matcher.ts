...
export function matchItemMods(
  ...
  for (const mod of isGemItem ? [] : explicits) {
    ...
    if (matched) {
      ...
      // For negative values, the trade API uses max (not min) to set the threshold.
      // e.g. -50% lightning res -> max: -50 finds -50 or worse (-63, etc.)
      const isNegative = matched.value != null && matched.value < 0
      const minValue =
        matched.value != null && !isNegative ? (isFixedValue ? matched.value : Math.floor(matched.value * pct)) : null
      const maxValue = isNegative ? matched.value : null
