...
export async function searchTrade(
  ...
  const searchResult = (await fetchJson(`https://www.pathofexile.com/api/trade/search/${encodeURIComponent(league)}`, {
    method: 'POST',
    body,
  ...
  const fetchResult = (await fetchJson(
    `https://www.pathofexile.com/api/trade/fetch/${ids}?query=${searchResult.id}`,
  )) as {
    result: Array<{
      id: string
      listing: {
...
export async function searchBulkExchange(
  ...
  const result = (await fetchJson(`https://www.pathofexile.com/api/trade/exchange/${encodeURIComponent(league)}`, {
    method: 'POST',
    body,
  })) as {
    id: string
