...
import { searchTrade, isBulkExchangeItem, getBulkExchangeId, searchBulkExchange } from '../trade'
import type { StatFilter, TradeResult, BulkExchangeResult } from '../trade'
...
async function clickTradeButton(
  ...
  const tradeUrl = `https://www.pathofexile.com/trade/search/${encodeURIComponent(league)}/${queryId}`
...
export function register(store: Store<AppSettings>): void {
  ...
  ipcMain.handle('poe-login', () => {
    ...
    loginWindow.webContents.on('did-navigate', (_event, url) => {
      if (url.includes('pathofexile.com/my-account') || url === 'https://www.pathofexile.com/') {
        loginWindow.close()
      }
    })
  })
