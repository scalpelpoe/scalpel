...
// Because `validateWindowPosition` calls `screen.getAllDisplays()` we must
// control the mock's return value. Path: electron's screen module.
const mockWorkAreas: Array<{ x: number; y: number; width: number; height: number }> = []

vi.mock('electron', () => ({
  screen: {
    getAllDisplays: () =>
      mockWorkAreas.map((wa) => ({
        workArea: wa,
      })),
  },
}))

// Load the module under test after the mock is installed.
let validateWindowPosition: typeof import('./app-window-position').validateWindowPosition

beforeEach(async () => {
  mockWorkAreas.length = 0
  // Dynamic import so the mock is active.
  const mod = await import('./app-window-position')
  validateWindowPosition = mod.validateWindowPosition
})

describe('validateWindowPosition', () => {
  it('returns the input position when it fits on the primary display', () => {
    mockWorkAreas.push({ x: 0, y: 0, width: 2560, height: 1440 })
    const result = validateWindowPosition({ x: 100, y: 100, width: 520, height: 550 })
    expect(result).toEqual({ x: 100, y: 100 })
  })

  it('returns the input position when on a secondary display with negative coords', () => {
    mockWorkAreas.push({ x: 0, y: 0, width: 1920, height: 1080 })
    mockWorkAreas.push({ x: -1920, y: 0, width: 1920, height: 1080 })
    const result = validateWindowPosition({ x: -1500, y: 100, width: 520, height: 550 })
  ...
  it('clamps to the nearest display when the stored position is completely off-screen', () => {
    ...
    const result = validateWindowPosition({ x: 3000, y: 200, width: 520, height: 550 })
  ...
  it('clamps when the window sits in a deadzone between two L-shaped monitors', () => {
    ...
    mockWorkAreas.push({ x: 0, y: 0, width: 1920, height: 1080 }) // primary
    mockWorkAreas.push({ x: 1920, y: 300, width: 1920, height: 1080 }) // secondary
    ...
    const result = validateWindowPosition({ x: 1900, y: 50, width: 520, height: 550 })
    // Should clamp either to primary or secondary work area.
    expect(result.x).toBeGreaterThanOrEqual(-0)
    // Verify the clamped position fits on some display.
    const fitsSomewhere = mockWorkAreas.some(
      (wa) =>
  ...
  it('preserves the position when the titlebar is draggable even if partially off-screen', () => {
    ...
    const result = validateWindowPosition({ x: 1800, y: 50, width: 520, height: 550 })
  ...
  it('clamps when the titlebar is nearly invisible (only 1px draggable)', () => {
    ...
    mockWorkAreas.push({ x: 0, y: 0, width: 1920, height: 1080 })
    const result = validateWindowPosition({ x: 100, y: -31, width: 520, height: 550 })
  ...
  it('clamps when the window is entirely outside every work area', () => {
    mockWorkAreas.push({ x: 0, y: 0, width: 1920, height: 1080 })
    const result = validateWindowPosition({ x: -9999, y: 5000, width: 520, height: 550 })
    expect(result.x).toBeGreaterThanOrEqual(0)
    expect(result.y).toBeGreaterThanOrEqual(0)
    expect(result.x + 520).toBeLessThanOrEqual(1920)
    expect(result.y + 550).toBeLessThanOrEqual(1080)
  ...
  it('clamps a window that is larger than the work area', () => {
    mockWorkAreas.push({ x: 0, y: 0, width: 800, height: 600 })
    const result = validateWindowPosition({ x: 500, y: 500, width: 1200, height: 900 })
  ...
  it('selects the display whose work area contains the stored center point', () => {
    ...
    const result = validateWindowPosition({ x: 1920, y: 50, width: 520, height: 550 })
    expect(result.x).toBeGreaterThanOrEqual(1920)
  })

  it('falls back to nearest display when center is in a deadzone', () => {
    mockWorkAreas.push({ x: 0, y: 0, width: 1920, height: 1080 })
    mockWorkAreas.push({ x: 2200, y: 0, width: 1920, height: 1080 }) // gap between 1920-2200
    ...
    const result = validateWindowPosition({ x: 2000, y: 200, width: 100, height: 550 })
