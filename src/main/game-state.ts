 ...
 *  modules would otherwise fail to load in Vitest. Set by createOverlayWindow at
 *  startup. In stable mode it is fixed for the process lifetime (game switches
 *  relaunch); in experimental multi-window mode the in-process switch updates it
 *  live via setPoeVersion, so always read through getPoeVersion() rather than
 *  caching the value.
 *
